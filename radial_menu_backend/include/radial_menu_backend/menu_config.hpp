...
struct MenuConfig {
  MenuConfig()
      : allow_multi_selection(false), reset_on_opening(false), reset_on_closing(false),
        auto_select(false), open_button(/* PS4's circle*/ 1), select_button(/* PS4's R1 */ 5),
  ...
  static MenuConfig fromParamNs(const std::string &ns) {
    ...
    nh.getParam("reset_on_opening", config.reset_on_opening);
    nh.getParam("reset_on_closing", config.reset_on_closing);
    nh.getParam("auto_select", config.auto_select);
    nh.getParam("open_button", config.open_button);
